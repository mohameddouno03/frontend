...
export default function DashboardLayout({ children }) {
  ...
  const [sidebarVisible, setSidebarVisible] = useState(true); // sidebar visible par défaut
  ...
  return (
    <div className="flex h-screen">
      {/* Sidebar fixe */}
      <div
        className={`bg-gray-100 shadow-lg h-screen flex flex-col justify-between transition-all duration-300 ${
          sidebarVisible ? "w-48" : "w-0 overflow-hidden"
        }`}
      >
        <div>
            ...
            {menuItems.map((item, idx) => (
              <li key={idx}>
                <a
                  ...
                  className="flex items-center gap-3 p-3 text-gray-700 hover:bg-gray-200 cursor-pointer rounded transition-colors"
                >
